...
class ThreadNeedleEnv(AVAlohaEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.needle_joint = self.mjcf_root.find('joint', 'needle_joint')
        self.wall_joint = self.mjcf_root.find('joint', 'wall_joint')
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        if (options and options.get('distractors', False)):
            ...
            while len(distractor_positions) < len(distractor_geoms):
                for i in range(max_tries):
                    ...
                    if np.linalg.norm(d_pos - needle_position) > min_distance and all(
        ...
        if (options and options.get('adverse', False)):
            ...
            while True:
                d_pos = np.random.uniform(ranges[:, 0], ranges[:, 1])
                if np.linalg.norm(d_pos - needle_position) > min_distance:
...
def main():
    ...
    env = gym.make("gym_av_aloha/thread-needle-v1", cameras={}, fps=8.33)
